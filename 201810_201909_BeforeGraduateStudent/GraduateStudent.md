...
### 2019年3月21日:
然后新改的程序需要跑的是60个节点下，用原来的数据，跑第一个出发机制和第二个出发机制以及RCSS,TADP和NJNP
不用跑第一个对比和第二个对比

### 2019年3月24日：
1、以双阈值实验为前提：当TADP等三个实验的缓冲池中出现发送了ALERT的节点，MCV立即出发为缓冲池中的节点进行充电，直到缓冲池的长度为0
2、以自适应缓冲池为前提：当TADP等三个实验的缓冲池长度达到自适应缓冲池的长度时，MCV立即出发为缓冲池中的节点进行充电，直到缓冲池的长度为0

行，大概理解了一些，就是在这三个实验里面做双阈值计算和自适应缓冲池的计算
