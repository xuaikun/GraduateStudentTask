...
Step1:
		...
		for(int i = 1; i <= n; i++)
		{
			for(int j = i + 1; j <= n; j++)
			{
				cout << "Clique[" << i << "]["<< j << "].EdgeDelete=" << Clique[i][j].Edge_Delete << endl; 
			}
		}
#endif
//_sleep(60000);		
...
#ifdef Debug
			cout << "*******************Step1**********************" << endl;
			cout << "最大度 max_Degree_Inceasing_Order = " <<max_Degree_Inceasing_Order << endl; 
			cout << "~~~~~~~~~~~~~~~~程序终止~~~~~~~~~~~~~~~~~~" << endl; 
			cout << "******************************************" << endl;
#endif
			goto end;
		...
		cout << "Clique[i][j].Common_Neighbor:原三角" << endl;
#endif
		...
		for(int i = 1; i <= n; i++)				//	只获取Common_Neighbor上三角部分数据，保证Common_Neighbor[][]只保存不同边的个数 
		{
			for(int j = 1; j <= n; j++)
			{
				...
				if(i <= j){
					cout <<Clique[i][j].Common_Neighbor_New  << " ";
				}
				else
				{
					cout << "  ";
				}
#endif		 //将Common_Neighbor的上三角提取出来 
				if(j == n)
				{
#ifdef Debug
					cout << endl;	
...
#ifdef Debug
		cout << "***********打印Common_Neighbor_Sort[i] *********" << endl; 
		for(int i = 1; i <= n; i++)
		{
		...
		for(int i = 1; i <= n; i++)
		{
			for(int j = i + 1; j <= n; j++)
			{
				...
				{
#ifdef Debug
					cout << "Common_Neighbor_Sort[1] = " << Common_Neighbor_Sort[1] << endl;
					cout << "i =" << i << endl;
					cout << "j =" << j << endl;
					cout <<	"Clique["<<i<<"]["<<j<<"].Common_Neighbor_New = " << Clique[i][j].Common_Neighbor_New <<endl;
					cout << "Clique["<<i<<"]["<<j<<"].EdgeList = "            << Clique[i][j].EdgeList  << endl;
#endif
...
#ifdef Debug
		cout << "Max_Common_Neighbor_num = " << Max_Common_Neighbor_num << endl;
#endif
		Max_Degree = 0;
		Min_Edge = 10000;
		...
		{
#ifdef Debug
			cout << "Common_Neighbor的最大的边不止一条" << endl; 
#endif
...
Step4:
#ifdef Debug
		cout << "开始通过**************************Step4*************************************"<<endl; 
		cout << "~~~~~~~~~~~~打印特殊点~~~~~~~~~~~~~~~~~" << endl; 
		cout << "选中的点Max_i = " << Max_i << endl;
#endif
...
Step5:
		...
		/*由于节点被修改，每个节点的度也被修改了 重新计算度的值*/ 
...
#ifdef Debug
				cout << "*****************Step5***********************" << endl;
				cout << "节点Max_i = " << Max_i << "已经单独存在！！！！" << endl;
				cout << "最大度 max_Degree_Inceasing_Order = " <<max_Degree_Inceasing_Order << endl; 
				cout << "~~~~~~~~~~~~~~~~程序终止~~~~~~~~~~~~~~~~~~" << endl; 
				cout << "******************************************" << endl;
#endif
			...
			cout << "**********************************" << endl;
		 	cout << "Max_i =" << Max_i << endl;
			cout << "Degree[" << Max_i << "] =" << Degree[Max_i] << endl; 
			cout << "**********************************" << endl;
#endif
			...
			else								    // 还有与Max_i相连的边 
			{
				...
				for(int j = 1; j <= n; j++)
				{
					...
					//找到最大Common_Neighbor的两个节点 防止最大Common_Neighbor为0时出现错误，保证选中的两个节点必须相连 
					{
						...
						Max_Common_Neighbor_j_Point[Max_Common_Neighbor_num] = j;			//Max_Common_Neighbor_num可以作为index 
#ifdef Debug
						cout << "Clique["<<Max_i<<"]["<<j<<"].Edge_Delete = " << Clique[Max_i][j].Edge_Delete << endl;
#endif
					}
				}	
				Max_Degree = 0;
