...
result_path = "E:\\00000000000graduate-study\\GraduateStudentTask\\201810_201909_BeforeGraduateStudent\\python_programming\\Ondemand_Charge_AdaptiveThreshold\\result"
# 以下为数据初始化
...
if __name__ == "__main__":
    ...
    if NJNPFlag is True:
        ...
        # El初始化为0
        El = 0
        # 初始化缓冲池阈值大小为RequestThreshlodMin
        RequestThreshold = RequestThresholdMin
        Simulation_time = 0.0
        print "NJNP Task"
        # 使用备份数据
            ...
            while OperateMNodeNum != NodeListNum:
                ...
                if NodeRequestFlag is False:
                    ...
                    for i in range(1, len(NodeList)):
                        ...
                        if  (len(Request_list) >= RequestThreshold) and (i == len(NodeList) - 1):
                            print "the length of Request_list >= RequestThreshold"
                            # 充电请求已经达到固定阈值上限，开始创建充电回路
                            NodeRequestFlag = True
                            # 判断是否是第一次进行构造充电回路
                            FirstComingFlag = True 
                ...
                while NodeRequestFlag is True:
                    ...
                    for i in range(1, len(R_list)):
                        ...
                        if MCVAndServiceNode_distance > N_distance[R_New[len(R_New) - 1]][R_list[i]]:
                            MCVAndServiceNode_distance = N_distance[R_New[len(R_New) - 1]][R_list[i]]
                            # 找出当前较优节点
                            BestNode = R_list[i]
                    # 本次寻找最好的节点为 BestNode
                    # 该点不再运动
    ...
    if TADPFlag is True:
        ...
        # El初始化为0
        El = 0
        # 初始化缓冲池阈值大小为RequestThresholdMin
        RequestThreshold = RequestThresholdMin
        Simulation_time = 0.0
        print "TADP Task"
        # 使用备份数据
            ...
            while OperateMNodeNum != NodeListNum:
                ...
                while NodeRequestFlag is True:
                    ...
                    for i in range(1, len(R_list)):
                        ...
                        Node_number_Es = []
                        Node_number_Es.append(R_list[i])
                        # 获取节点剩余能量
                        Node_number_Es.append(round(NodeEs[0][R_list[i]],2))
                        ...
                        Node_number_distance.append(R_list[i])
                        # 获取MCV到当前节点的距离
                        Node_number_distance.append(round(N_distance[R_New[len(R_New) - 1]][R_list[i]], 2))
                    ...
                    for i in range(1 , len(R_list)):
                        ...
                        P_Node_number = []
                        # 添加节点序号
                        P_Node_number.append(R_list[i])
                        Es_Rank = 0
                        for l in range(1, len(R_list)):
                            if R_list[i] == Es_sort[l - 1][0]:
                                # 当前剩余能量节点的排名
                                Es_Rank = l
                                break
                        Distance_Rank = 0
                        for m in range(1, len(R_list)):
                            if R_list[i] == N_distance_sort[m - 1][0]:
                                # 当前 节点距离S的排名
                                Distance_Rank = m
                                break
            ...
            while OperateMNodeNum != NodeListNum:
                ...
                while NodeRequestFlag is True:
                    ...
                    for i in range(1, len(R_list)):
                        ...
                        Node_number_ReciprocalP.append(R_list[i])
                        # 获取功耗的倒数【因为需要从将功耗从大到小，为了便于我的计算，用了
                        # 功耗的倒数，排序时，从小达到大排序】
                        Node_number_ReciprocalP.append(round((1.0/NodeP[0][R_list[i]]),4))
                    ...
                    for i in range(1 , len(R_list)):
                        ...
                        ReciprocalP_Rank = 0
                        for l in range(1, len(R_list)):
                            if R_list[i] == NodeReciprocalP_sort[l - 1][0]:
                                # 当前剩余能量节点的排名
                                ReciprocalP_Rank = l
                                break
                        Distance_Rank = 0
    ...
    plt.plot(data[0], data[1],'c-s',label = 'RespondTime1')
    ...
    plt.plot(data[0], data[2], 'c-s',label = 'ServiceTime1')
